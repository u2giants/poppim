...
| partial | Product cover image storage | 2026-06-12: chose **DigitalOcean Spaces** (S3, public Space `poppim` @ `nyc3`) over R2 for product covers. `pm-system/migration/clickup-images-recover.mjs` re-fetches working ClickUp URLs (the `_large` thumbnails ClickUp disabled now 403); `pm-system/migration/spaces-cover-upload.mjs` downscales (sharp → webp ≤1000px) and uploads to the Space, then repoints `product.cover_url` at `https://poppim.nyc3.digitaloceanspaces.com/covers/<id>.webp`. Creds in mode-600 `~/.directus-deploy.env` (`DO_SPACES_*`). Directus's own file storage is still local; design files stay on NAS. |
